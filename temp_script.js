...
document.addEventListener('visibilitychange',()=>{if(document.hidden)paused=activeLoop,activeLoop='none';else activeLoop=paused;});
let paused='gate-cv';
...
genericBGs.forEach(bg=>{
  ...
  (function go(){
    requestAnimationFrame(go);
    if(activeLoop!==bg.id)return;
    t+=.008;
    bg.draw(ctx,cv.width,cv.height,t);
  })();
});

// â”€â”€â”€ GATE CANVAS (already initialised above but add star-ring) â”€â”€â”€â”€â”€â”€â”€â”€
